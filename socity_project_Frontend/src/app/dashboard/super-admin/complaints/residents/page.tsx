...
export default function ResidentComplaintsPage() {
    const { data: response, isLoading } = useQuery({
        queryKey: ['super-admin-resident-complaints'],
        queryFn: () => ComplaintService.getAll()
    })

    const allComplaints = response?.data || (Array.isArray(response) ? response : [])
    const residentComplaints = allComplaints.filter((c: any) => c.source === 'resident' || c.reportedByOriginal?.role === 'RESIDENT' || !c.source)
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Resident Complaints</h1>
                        <p className="text-gray-500 dark:text-gray-400">Service issues reported by Residents</p>
                    </div>
                </div>
